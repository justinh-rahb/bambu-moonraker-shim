...
class DatabaseManager:
    ...
    def ensure_temperature_panel(self):
        shim_meta = self._db.setdefault("_shim", {})
        if shim_meta.get("temperature_panel_seeded"):
            return

        mainsail = self._db.setdefault("mainsail", {})
        updated = False

        def ensure_panel(layout_key: str):
            nonlocal updated
            layout = mainsail.get(layout_key)
            if layout is None:
                layout = []
            if isinstance(layout, list):
                if not any(isinstance(panel, dict) and panel.get("name") == "temperature" for panel in layout):
                    mainsail[layout_key] = [{"name": "temperature", "visible": True}] + list(layout)
                    updated = True

        ensure_panel("dashboard.desktopLayout1")
        ensure_panel("dashboard.tabletLayout1")

        shim_meta["temperature_panel_seeded"] = True
        self._save()
